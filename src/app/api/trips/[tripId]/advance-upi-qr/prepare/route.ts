...
export async function POST(
  ...
  if (!ALLOWED_UPI_QR_MIME_TYPES.has(mimeType.toLowerCase())) {
    return NextResponse.json({ ok: false, message: "UPI QR must be JPG, PNG, or WEBP image" }, { status: 400 });
  }
  if (!ALLOWED_UPI_QR_EXTENSIONS.has(fileExt)) {
    return NextResponse.json({ ok: false, message: "UPI QR extension must be jpg, jpeg, png, or webp" }, { status: 400 });
  }
  if (fileSizeBytes <= 0 || fileSizeBytes > MAX_UPI_QR_SIZE_BYTES) {
    return NextResponse.json({ ok: false, message: "UPI QR size must be between 1 byte and 10 MB" }, { status: 400 });
  }

  const prepareResult = await actorResult.supabase.rpc("trip_advance_upi_qr_prepare_v1", {
  ...
  if (prepareResult.error) {
    if (isMissingRpcError(prepareResult.error)) {
      return NextResponse.json({ ok: false, message: "Missing RPC: trip_advance_upi_qr_prepare_v1" }, { status: 500 });
    }
    return mapRpcError(prepareResult.error.message ?? "Unable to prepare UPI QR upload", prepareResult.error.code);
  }
  ...
  if (!canonicalObjectKey) {
    return NextResponse.json({ ok: false, message: "Unable to prepare UPI QR upload" }, { status: 500 });
  }
    ...
    return NextResponse.json(
      {
        ...
        message:
          payload?.error ||
          "Unable to prepare UPI QR upload URL from worker. Ensure worker allows docType 'upi_qr'.",
      },
      { status: 502 },
    );
  }
