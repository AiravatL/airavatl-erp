...
export async function POST(
  ...
  const body = (await request.json().catch(() => null)) as FinalPaymentBody | null;
  ...
  const hasAnyBankField = Boolean(bankAccountHolder || bankAccountNumber || bankIfsc || bankName);
  const hasUpiQrMetadata = Boolean(upiQrFileName || upiQrMimeType || upiQrSizeBytes !== null);

  if (paymentMethod === "bank") {
    if (hasAnyBankField) {
      if (!bankAccountHolder || !bankAccountNumber || !bankIfsc || !bankName) {
        return NextResponse.json({ ok: false, message: "Bank details are incomplete" }, { status: 400 });
      }
  ...
  if (paymentMethod === "upi") {
    ...
    if (!upiQrObjectKey && hasUpiQrMetadata) {
      return NextResponse.json(
        { ok: false, message: "UPI QR object key is required when QR metadata is provided" },
        { status: 400 },
      );
    }
    if (upiQrObjectKey) {
  ...
  const { data: rpcData, error: rpcError } = await actorResult.supabase.rpc("trip_final_payment_request_create_v1", {
  ...
  if (rpcError) {
    if (isMissingRpcError(rpcError)) {
      return NextResponse.json({ ok: false, message: "Missing RPC: trip_final_payment_request_create_v1" }, { status: 500 });
    }
    return mapRpcError(rpcError.message ?? "Unable to create final payment request", rpcError.code);
  }
