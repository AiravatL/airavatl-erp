...
export async function POST(
  ...
  const { data: rpcData, error: rpcError } = await actorResult.supabase.rpc("trip_pod_proof_confirm_v1", {
  ...
  if (rpcError) {
    if (isMissingRpcError(rpcError)) {
      return NextResponse.json({ ok: false, message: "Missing RPC: trip_pod_proof_confirm_v1" }, { status: 500 });
    }
    return mapRpcError(rpcError.message ?? "Unable to confirm POD upload", rpcError.code);
  }
