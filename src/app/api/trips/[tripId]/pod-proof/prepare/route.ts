...
export async function POST(
  ...
  const prepareResult = await actorResult.supabase.rpc("trip_pod_proof_prepare_v1", {
  ...
  if (prepareResult.error) {
    if (isMissingRpcError(prepareResult.error)) {
      return NextResponse.json({ ok: false, message: "Missing RPC: trip_pod_proof_prepare_v1" }, { status: 500 });
    }
    return mapRpcError(prepareResult.error.message ?? "Unable to prepare POD upload", prepareResult.error.code);
  }
