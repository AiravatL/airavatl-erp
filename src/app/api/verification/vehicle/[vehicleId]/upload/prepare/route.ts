...
export async function POST(
  ...
  if (!fileName || !mimeType) {
    return NextResponse.json(
      { ok: false, message: "fileName and mimeType are required" },
      { status: 400 },
    );
  }

  const { data: rpcData, error: rpcError } = await actorResult.supabase.rpc(
    "verification_prepare_vehicle_rc_upload_v1",
    {
  ...
  if (rpcError) {
    if (isMissingRpcError(rpcError)) {
      return NextResponse.json(
        { ok: false, message: "Missing RPC: verification_prepare_vehicle_rc_upload_v1" },
        { status: 500 },
      );
    }
  ...
  if (!objectKey) {
    return NextResponse.json(
      { ok: false, message: "Unable to prepare upload" },
      { status: 500 },
    );
  }
  ...
  if (!accessToken) {
    return NextResponse.json(
      { ok: false, message: "Missing worker auth context" },
      { status: 500 },
    );
  }
  ...
  const payload = (await presignResponse
    .json()
    .catch(() => null)) as WorkerPresignPutResponse | null;
