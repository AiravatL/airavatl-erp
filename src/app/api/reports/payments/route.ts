...
export async function GET(request: Request) {
  ...
  const paymentType = searchParams.get("paymentType")?.trim() || null;

  const { data: rpcData, error: rpcError } = await actorResult.supabase.rpc(
    "admin_list_payments_v1",
    {
  ...
  if (rpcError) {
    if (isMissingRpcError(rpcError)) {
      return NextResponse.json({ ok: false, message: "Missing RPC: admin_list_payments_v1" }, { status: 500 });
    }
    return NextResponse.json({ ok: false, message: rpcError.message ?? "Unable to fetch payments" }, { status: 500 });
  }
