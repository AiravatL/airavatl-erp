...
export async function GET(request: Request) {
  ...
  const { data: rpcData, error: rpcError } = await actorResult.supabase.rpc("report_receivables_aging_v1", {
  ...
  if (rpcError) {
    if (isMissingRpcError(rpcError)) {
      return NextResponse.json({ ok: false, message: "Missing RPC: report_receivables_aging_v1" }, { status: 500 });
    }
    return mapReportRpcError(rpcError.message ?? "Unable to fetch receivables aging", rpcError.code);
  }

  const normalized = Array.isArray(rpcData) ? (rpcData[0] ?? null) : rpcData;
  return NextResponse.json({ ok: true, data: normalized ?? { summary: {}, buckets: [], customers: [] } });
}
