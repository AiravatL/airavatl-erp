...
export async function GET(request: Request) {
  ...
  const { searchParams, search, limit, offset } = parsePaginationParams(request);
  ...
  if (rpcError) {
    if (isMissingRpcError(rpcError)) {
      return NextResponse.json({ ok: false, message: "Missing RPC: admin_consigner_list_v1" }, { status: 500 });
    }
    return NextResponse.json({ ok: false, message: rpcError.message ?? "Unable to fetch customers" }, { status: 500 });
  }
