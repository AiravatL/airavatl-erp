...
export async function GET(request: Request) {
  ...
  const { data: rpcData, error: rpcError } = await actorResult.supabase.rpc("report_fuel_variance_v1", {
  ...
  if (rpcError) {
    if (isMissingRpcError(rpcError)) {
      return NextResponse.json({ ok: false, message: "Missing RPC: report_fuel_variance_v1" }, { status: 500 });
    }
    return mapReportRpcError(rpcError.message ?? "Unable to fetch fuel variance", rpcError.code);
  }
