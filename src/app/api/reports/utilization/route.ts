...
export async function GET(request: Request) {
  ...
  const { data: rpcData, error: rpcError } = await actorResult.supabase.rpc("report_utilization_v1", {
  ...
  if (rpcError) {
    if (isMissingRpcError(rpcError)) {
      return NextResponse.json({ ok: false, message: "Missing RPC: report_utilization_v1" }, { status: 500 });
    }
    return mapReportRpcError(rpcError.message ?? "Unable to fetch utilization", rpcError.code);
  }
