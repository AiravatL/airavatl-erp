...
export async function GET(request: Request) {
  ...
  const consignerProfileId = new URL(request.url).searchParams.get("consignerProfileId")?.trim() ?? "";
  if (!consignerProfileId) {
    return NextResponse.json({ ok: false, message: "consignerProfileId is required" }, { status: 400 });
  }
  ...
  if (error) {
    ...
    return NextResponse.json({ ok: false, message: error.message ?? "Unable to fetch bulk payment proof upload" }, { status: 500 });
  }
