...
import {
  confirmTripPodProofUpload,
  prepareTripPodProofUpload,
} from "@/lib/api/trips";
...
export function PodProofUploadDialog({ tripId, onClose, onSuccess }: Props) {
  ...
  const uploadMutation = useMutation({
    mutationFn: async () => {
      ...
      await prepareAndUploadSingleFile({
        ...
        prepare: (payload) => prepareTripPodProofUpload(tripId, payload),
        confirm: (payload) => confirmTripPodProofUpload(tripId, payload),
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-base">Upload POD</DialogTitle>
        </DialogHeader>
